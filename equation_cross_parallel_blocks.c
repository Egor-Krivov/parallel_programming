...
double next_value_cross(double left_value, double down_value, double right_value, double f_value, double h, double tau) {
    double value = (left_value - right_value) / h +  2 * f_value;
    value *= tau;
    value += down_value;
    return value;
}
            ...
            if (i == 1) {
                ...
                if (j == 0) {
                    if (rank == 0)
                        ...
                        f_value = f(left_bound * h, i * tau);
                        data[i][j] = next_value_cross(left_value, data[i - 2][j], data[i - 1][j + 1], f_value, h, tau);
                    }
                }
                else if (j == (columns_needed - 1)) {
                    if (rank == (size - 1)) {
                        f_value = f((left_bound + j) * h, i * tau);
                        data[i][j] = next_value_triangle(data[i - 1][j - 1], data[i - 1][j], f_value, h, tau);
                    }
                        ...
                        f_value = f((left_bound + j) * h, i * tau);
                        data[i][j] = next_value_cross(data[i - 1][j - 1], data[i - 2][j], right_value, f_value, h, tau);
                    }
                }
                else {
                    f_value = f((left_bound + j) * h, i * tau);
                    data[i][j] = next_value_cross(data[i - 1][j - 1], data[i - 2][j], data[i - 1][j + 1], f_value, h, tau);
                }
            }
        }
        if (i != num_t) {
            ...
            if (rank != 0)
                MPI_Recv(&left_value, 1, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, &status);
            if (rank != 0)
                MPI_Send(&data[i][0], 1, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD);
            if (rank != (size - 1))
                MPI_Recv(&right_value, 1, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD, &status);
        }
    }
