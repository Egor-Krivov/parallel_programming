...
int main(int argc, char* argv[]) {
    ...
    for (i = 1; i <= num_t; i++) {
        for (j = 1; j < num_x; j++) {
            f_value = f(j * h, i * tau);
            if (i == 1)
                grid[i][j] = next_value_triangle(grid[i - 1][j - 1], grid[i - 1][j], f_value, h, tau);
            else 
                grid[i][j] = next_value_cross(grid[i - 1][j - 1], grid[i - 2][j], grid[i - 1][j + 1], f_value, h, tau);
        }
        f_value = f(MAX_X, i * tau);
        grid[i][num_x] = next_value_triangle(grid[i - 1][num_x - 1], grid[i - 1][num_x], f_value, h, tau);
    }
